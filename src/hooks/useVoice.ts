...
export function useVoice() {
  ...
  const parseExpenseFromTranscript = (txt: string) => {
    ...
    const categoryKeywords = ["food", "transport", "airtime", "misc", "miscellaneous"];
    ...
    if (vendorMatch && vendorMatch[1]) {
      ...
      if (expensePhraseMatch && expensePhraseMatch[1]) {
        const potentialVendorPhrase = expensePhraseMatch[1].replace(/\b(food|transport|airtime|misc|miscellaneous)\b/g, '').trim();
